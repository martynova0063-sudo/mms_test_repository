...
Это документ ‑ `паспорт` для ML‑модели или расчётного алгоритма: он фиксирует, что делает модель, в каком статусе она находится, какие у неё ограничения и как её безопасно использовать. 
