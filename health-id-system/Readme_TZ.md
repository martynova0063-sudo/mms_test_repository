...
Взаимодействие медицинского и корпоративного контуров строго разграничено и осуществляется через защищённый API с минимальным набором данных.

```mermaid
flowchart LR
    subgraph MED["🏥 Медицинский контур"]
        direction TB
        M1["Витальные данные ПрМО"]
        M2["Расчёт HEALTH_ID"]
        M3["Human review<br/>(медработник)"]
        M4["Evidence + аудит"]
    ...
    MED -.->| "🔒 санкционированный API<br/>минимальный объём данных" | CORP
```
