...
│  🚫 Не медицинское изделие — только исследовательский      │
│     контекст до завершения валидации R0–R4               │
└─────────────────────────────────────────────────────────┘
...
text
    ...
    Вход: обезличенные данные ПрМО
           │
           ▼
    ┌──────────────────┐     ┌──────────────────┐
    │  Расчёт HEALTH_ID │     │  Валидация R0–R4  │
    │  (версионированный)│ ──▶│  (аналитическая →  │
    │                  │     │   клиническая)      │
    └──────────────────┘     └────────┬─────────┘
                                      │
                              ▼
...
text
┌─────────────────────────────────────────────────────────────────────────┐
│                        СИСТЕМА (Replit Project)                         │
│                                                                         │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐    │
│  │  Модуль 1   │  │  Модуль 2   │  │  Модуль 3   │  │  Модуль 4   │    │
│  │  Биометр.   │  │  HEALTH_ID  │  │  Приём      │  │  Baseline   │    │
│  │  верификация│  │  движок     │  │  данных     │  │  коридоры   │    │
│  └──────┬──────┘  └──────┬─────┘  └──────┬─────┘  └──────┬─────┘    │
│         │                │                │                │          │
│         ▼                ▼                ▼                ▼          │
│  ┌─────────────────────────────────────────────────────────┐           │
...
│              ▼             ▼              ▼                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                 │
│  │  Модуль 7    │  │  Модуль 8    │  │  API-слой    │                 │
│  │  Human Review│  │  Model       │  │  (FastAPI)   │                 │
│  │              │  │  Governance  │  │              │                 │
│  └──────────────┘  └──────────────┘  └──────────────┘                 │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────┐           │
│  │              PostgreSQL · Audit Log · Vault              │           │
│  └─────────────────────────────────────────────────────────┘           │
└─────────────────────────────────────────────────────────────────────────┘
...
│                                                                 │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────┐  │
│  │  Острое          │  │  Устойчивое      │  │  Хроническое │  │
│  │  отклонение      │  │  повторное       │  │  (подтв.)    │  │
│  │                  │  │  отклонение      │  │              │  │
│  │  Единичное рез-  │  │  ≥ N отклонений  │  │  Только из   │  │
│  │  кое превышение  │  │  по уникальным   │  │  МИС/ЭМК     │  │
│  │  коридора в     │  │  датам (не по    │  │              │  │
│  │  одном событии  │  │  числу попыток)  │  │  Источник:   │  │
│  │                  │  │                  │  │  не HEALTH_ID│  │
│  │  ⚡ Сразу в     │  │  📅 По датам,    │  │  📋 Внешний   │  │
│  │  human review   │  │  не по попыткам  │  │  мед. контур │  │
│  └──────────────────┘  └──────────────────┘  └──────────────┘  │
└─────────────────────────────────────────────────────────────────┘
...
text
    ...
    │            │                                     │
    │            └──▶ PENDING (запрошена доп. инфо)     │
    │                                                  │
    │  CONFIRMED ──▶ (закрыт)                          │
    │  REJECTED  ──▶ (закрыт, переход к повтору)       │
    └──────────────────────────────────────────────────┘
...
text
R0                R1                  R2                  R3                R4
Аналитическая  →  Ретроспективная  →  Внешняя          →  Клиническая    →  Финальная
валидация         валидация           проспективная      полезность        оценка
                                     валидация

Проверка         Проверка на         Проверка на        Оценка            Решение о
формул и         пилотном           независимом        влияния на        применении
логики           датасете            датасете           решения           в практике
(2967 соб.)      (2967 соб.)         (внешний набор)    медработника

Цель:            Цель:              Цель:              Цель:             Цель:
воспроизводи-    метрики             обобщаемость       практическая      готовность
мость,           качества на         на новые           польза для        к клиническому
корректность     доступных          данные             медработника      применению
формул           данных
...
17. План работ и контрольные точки 📅
...
18. Дисклеймеры и ограничения ⚠️
