...
Это документ‑«паспорт» для ML‑модели или расчётного алгоритма: он фиксирует, что делает модель, в каком статусе она находится, какие у неё ограничения и как её безопасно использовать. В проектe HEALTH_ID это способ обеспечить прозрачность, воспроизводимость и соответствие регуляторным требованиям (особенно важно для медицинских расчётов).
...
**Human review и объяснимость**. Карточка модели — это формальный способ зафиксировать, что расчёт требует проверки медработником и не является автономным диагнозом.
...
**Валидация R0–R4**. Каждая стадия валидации привязывается к конкретной версии модели: ты можешь сказать «валидация R2 проведена для v1.0.0 с весами 0.6/0.25/0.15».
